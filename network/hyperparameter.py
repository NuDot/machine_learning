...
def createModel(trainX, testX, trainY, testY):
  model = Sequential()
  model.add(Conv2D({{choice([32, 48])}}, (4, 4), padding='same',data_format="channels_first", input_shape=(30,50,25))) #h=100, w=200
  model.add(BatchNormalization(axis=1))
  model.add(Activation('relu'))
  model.add(MaxPooling2D(pool_size=(2, 2),strides=(2,2))) #h = 5, w = 10
  model.add(Dropout({{uniform(0, 1)}}))
  
  model.add(Conv2D({{choice([48, 128])}}, (3, 3), activation='relu')) #h=5-2+1=4, w = 10-3+1=8
  model.add(BatchNormalization())
  model.add(MaxPooling2D(pool_size=(2, 2), strides=(2,2))) #h=2, w=4
  model.add(Dropout({{uniform(0, 1)}}))
  
  model.add(Conv2D({{choice([64, 256])}}, (2, 3), padding='same', activation='relu')) #h=2, w=4
  model.add(BatchNormalization())
  model.add(MaxPooling2D(pool_size=(2, 2))) #h=1, w=2
  model.add(Dropout({{uniform(0, 1)}}))

  model.add(Conv2D({{choice([80,  512])}}, (2, 2), padding='same', activation='relu')) #h=2, w=4
  model.add(BatchNormalization())
  model.add(MaxPooling2D(pool_size=(2, 2))) #h=1, w=2
  model.add(Dropout({{uniform(0, 1)}}))
