...
def s2_rotation(x, a, b, c):
    ...
    x = np.repeat(x, np.shape(x)[-2], axis=-1) ############################################################
...
def plot(x, text, normalize=False):
    #assert x.size(0) == 1
    assert K.int_shape(x)[0] == 1
    #assert x.size(1) in [1, 3]
    assert K.int_shape(x)[1] in [1, 3]
    ...
    ''' #normalize function is not used yet. Need modification if needed
    if normalize:
