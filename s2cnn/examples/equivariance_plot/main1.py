...
def main():
    ...
    # load image
    x = imread("/project/snoplus/s2cnn/examples/equivariance_plot/earth128.jpg").astype(np.float32).transpose((2, 0, 1)) / 255
    ...
    plt.subplot(2, 3, 2)
    plot(phi(x), "f(x) : correlation", True)

    plt.subplot(2, 3, 3)
    plot(so3_rotation(phi(x), *abc), "R(f(x))", True)

    plt.subplot(2, 3, 5)
    plot(s2_rotation(x, *abc), "R(x) : rotation")

    plt.subplot(2, 3, 6)
    plot(phi(s2_rotation(x, *abc)), "f(R(x))", True)

    plt.tight_layout()
    plt.savefig("fig_final_project.jpeg")
