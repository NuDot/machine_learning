...
loss_im = loss_ax.imshow(loss, cmap='summer', interpolation='nearest')
acc_im = acc_ax.imshow(acc, cmap='autumn', interpolation='nearest', norm=matplotlib.colors.Normalize(vmin=0.5,vmax=1))
test_scale = test_ax_scale.imshow(np.transpose(test_scale),cmap='autumn', interpolation='nearest')
loss_scale = loss_ax_scale.imshow(np.transpose(loss_scale),cmap='summer', interpolation='nearest')
plt.show()
