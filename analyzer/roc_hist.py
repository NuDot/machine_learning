...
for time in range(N_TIMES):
  for qe in range(N_QES):
    ##################################################
    if (HANDLE == 1):
      path = '/projectnb/snoplus/sphere_data/Xe136_C10_random_clock/Xe136C10time%d_qe%d/Xe136_C10_qe%d_time%d_*_roc_value.npy' % (time, qe, 10, 8)
      #path = '/projectnb/snoplus/sphere_data/Xe136_C10_bo_v2/Xe136C10time%d_qe%d/Xe136_C10_*_roc_value.npy' % (time, qe)
    elif (HANDLE == 2):
      path = '/projectnb/snoplus/sphere_data/Xe136_C10_buffer_oil_30epoch_center/Xe136C10time%d_qe%d/Xe136_C10_*_roc_value.npy' % (time, qe)
    else:
...
loss_hist = loss[np.where(loss !=0.0)].flatten()
print(loss_hist)
mu = 61.6 * 1e-2
sigma = 1.9 * 1e-2
x = np.linspace(mu - 3*sigma, mu + 3*sigma, 100)
plt.plot(x, stats.norm.pdf(x, mu, sigma), label='Reported Result')
plt.hist(loss_hist, bins=10, label='Trained Result')
print(np.average(loss_hist))
assert 0
plt.axvspan(xmin=mu - sigma, xmax=mu + sigma, color='grey', alpha=0.1, label='1-Sigma')
plt.legend()
plt.xlabel('Rejection Rate at 90% Acceptance')
plt.savefig('rc.png')
plt.show()
assert 0
