...
qe_tick = np.linspace(100,14, N_QES, )
...
for time in time_range:
  for qe in qe_range:
    if (HANDLE == 1):
      path = '/projectnb/snoplus/sphere_data/Xe136_C10/Xe136C10time%d_qe%d/Xe136_C10_qe%d_time%d_*_model.h5' % (time, qe, qe, time)
      param_path = '/projectnb/snoplus/sphere_data/Xe136_C10/Xe136C10time%d_qe%d/Xe136_C10_qe%d_time%d_*_roc_param.npy' % (time, qe, qe, time)
    elif (HANDLE == 2):
    ...
    while(len(qestr) < 5):
      qestr = qestr + '  '
    timestr = str(100.0 - 10.0 * time)
    while(len(timestr) < 5):
      timestr += '  '
    plt.plot((1-fpr), tpr, label = ' ' + timestr + '       ' + qestr + '     %.3f'%(effpurity))
