...
time_tick = np.linspace(100,20,9)
...
for time in range(N_TIMES):
  for qe in range(N_QES):
    ##################################################
    if (HANDLE == 1):
      path = '/projectnb/snoplus/sphere_data/Xe136_C10/Xe136C10time%d_qe%d/Xe136_C10_qe%d_time%d_*_roc_value.npy' % (time, qe, qe, time)
    elif (HANDLE == 2):
      path = '/projectnb/snoplus/sphere_data/Xe136_C10_center/Xe136C10time%d_qe%d/Xe136_C10_qe%d_time%d_*_roc_value.npy' % (time, qe, qe, time)
...
if (HANDLE == 1):
  #plt.tight_layout()
  plt.savefig( PLOT_DIR  + 'Pressure_map_3m_Sphere.pdf', format='pdf', dpi=1000)
  plt.show()
  #np.save('1elROCPM.npy', loss)
elif (HANDLE == 2):
  #plt.tight_layout()
  plt.savefig( PLOT_DIR  + 'Pressure_map_center.pdf', format='pdf', dpi=1000)
  plt.show()
